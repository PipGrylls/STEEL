...
| 7 | Dynamical-friction merging time-scale, 2-panel (vs M_h,sat and M*,sat), 3 host masses | fn | **built, structure not verified against the real 2-panel/3-host-mass layout** (`Paper1_Fig7_MergerTimescale.png`) -- needs a check against the actual figure |
...
| 10 | SFR-M* relation at z=0.5/1/2 vs Leja+2019 | run + data | **not built** |
| 11 | Elliptical fraction vs stellar mass, 3 redshifts, vs SDSS T-Type | run + data | **needs re-investigation** -- previously marked infeasible based on `P_Elliptical` (confirmed dead in `STEEL.py`'s main loop), but the real Fig. 11 is a *post-processing* computation (major-merger-fraction from central mass tracks, mass ratio > 1/3 since z=3), not that array. May be buildable with the same mass-track machinery as Figs. 6/7/8. |
