...
| 13 | sSFR distributions, T16 vs CE, vs SDSS | run + data | **FIXED 2026-08-13** (`Paper1_Fig13_sSFRSweep.png`: same fix as Paper 2 Fig. 9 -- `ssfr_sfr_sweep.py` rewritten for multi-seed stochastic ensembles and a real step histogram, rebuilt with 8-seed stochastic runs of both `p1-sf-strip-t16` and `p1-sf-strip-ce` (`log Mh = 11.0-14.5`, same grid tradeoff as Fig. 9). Real CE vs T16, G18 preset; both now show the expected quenched/star-forming bimodal shape at the two lower mass columns (log M*≈9.5, 10.5), tight py/rust agreement; the log M*≈11.0 column is thin for the same grid-extent reason as Fig. 9's third panel. SDSS overlay omitted.) |
