...
| 13 | sSFR distributions, T16 vs CE, vs SDSS | run + data | **likely has the same deterministic-mode defect as Paper 2 Fig. 9, not yet rebuilt** (`Paper1_Fig13_sSFRSweep.png`: real CE vs T16, G18; SDSS overlay omitted; this was previously logged as "done" with a "small-number-statistics" caveat, but that assessment predates the 2026-08-13 finding that this whole class of sSFR-distribution figure needs stochastic mode -- deterministic mode collapses satellites onto the gas-depletion floor rather than producing genuine small-number noise. `ssfr_sfr_sweep.py` also has the same masked-line-instead-of-histogram plotting bug fixed in `ssfr_plot.py`. Needs the same treatment; not yet done.) |
...
| 8 | Mass tracks (cmodel), same 3-row structure as Fig. 6 | run | **done** (`Paper2_Fig8_MassTrackDecomposition.png`: `mass_tracks.py` gained a `--preset g19_cmod` option, `dump_mass_tracks.rs` a `cmod` arg; the M*=10^11 track shows SFH exceeding Total (fractional contribution >1, accretion going negative, large swings in the rate-ratio row) -- this isn't a bug, it reproduces the paper's own stated finding for this figure: cmodel's flatter high-mass SMHM slope makes the implied stellar-mass growth history *internally non-physical*, with accretion sometimes required to be negative to balance the budget. Tight py/rust agreement throughout, including in the pathological region.) |
| 9 | sSFR of satellites/centrals vs SDSS, 3 mass bins (10-10.5, 10.5-11.3, 11.3-12.5) | run + data | **FIXED 2026-08-13, satellite side; central side still missing** (`Paper2_Fig9_sSFR.png` was built in deterministic mode, which cannot reproduce this figure: the real Fig. 9 is a smooth, bimodal sSFR distribution that only exists because of the model's 0.3 dex Monte Carlo SFR scatter, and without it nearly every satellite in a bin converges onto the same gas-depletion floor (sSFR clamped to exactly 1e-12/yr in `Functions_c.pyx`), producing one dominant spike and nothing else -- user-caught, confirmed by rendering the actual published page. Rebuilt with `ssfr_plot.py` (rewritten to average several seed directories and plot a proper step histogram instead of a masked connect-the-dots line) over an 8-seed stochastic ensemble via `three_way.py --mode stochastic`, grid `log Mh = 11.0-14.5` (a full published-grid single-seed timing run came back at 20 minutes -- an 8-seed published-grid ensemble was not worth the wall-clock for a validation figure). The two-peak quenched/star-forming shape now matches the paper's qualitatively in the two lower mass bins, with tight py/rust agreement; the third (highest, 11.3-12.5) panel is thin/noisy rather than empty like before, because that grid doesn't reach the very massive parent haloes needed to populate it well -- would need the full published grid to fix properly. Central-galaxy dynamical-quenching line and SDSS overlay still not built.) |
