...
| 9 | sSFR of satellites/centrals vs SDSS, 3 mass bins (10-10.5, 10.5-11.3, 11.3-12.5) | run + data | **satellite side done, central side still missing** (`Paper2_Fig9_sSFR.png`: exact published mass ranges, summed over the satellite-mass axis within each range rather than a single nearest-bin snapshot; tight py/rust agreement in the two lower-mass panels, the top mass range is too sparse on this grid to say much; central-galaxy dynamical-quenching line and SDSS overlay not built; sparse-bin noise is small-number statistics, not a port defect) |
