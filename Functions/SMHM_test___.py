...
def DM_to_SM_Behr (x, e3, M3, alpha3):
    #==================================================
    """if set==1:
...
#for i in idx:
    ...
    SM_Beh_=np.zeros(4)
    for j in range(0,4):
        func = interp1d(z_Beh, SM_Beh[:,j])
        SM_Beh_[j] = func(z_Beh_)[j]
    DM_ = np.array([DM[idx[0]], DM[idx[1]], DM[idx[2]], DM[idx[3]]])
    popt, pcov = curve_fit(DM_to_SM_Behr, xdata=DM_, ydata=SM_Beh_, p0=p0)#, sigma=np.repeat(0.15,SM_Beh_.size))
    fit_params.append(popt)
    print(popt)
k=0
for i in idx:
    if k==0:
        plt.semilogx(z_Beh_, DM_to_SM_Behr(HM_to_fit[:,i], fit_params[k][0], fit_params[k][1], fit_params[k][2]), '-.', color='green', label='curve_fit')
    else:
        plt.semilogx(z_Beh_, DM_to_SM_Behr(HM_to_fit[:,i], fit_params[k][0], fit_params[k][1], fit_params[k][2]), '-.', color='green')
    k+=1
np.savetxt('fitted_params.txt', np.array(fit_params), header='Columns: e3, M3, alpha3, beta2, gamma2')
