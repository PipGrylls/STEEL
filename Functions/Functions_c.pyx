        ...
        for i in range(N):
            ...
            if T_quench[k] < t[i] or i == 0:
                ...
                if SFR_Model_int == 6:
                    ...
                    Norm = c_pow(10, 0.69+ 0.71*z[i] - 0.085*(z[i]**2)) #SFR peak
                    Alpha = 1.0 - 0.022*z[i] + 0.007*(z[i]**2) #low mass slope 
                    Beta = 1.8 - 0.7*z[i] - 0.035*(z[i]**2)#high mass slope
...
def Starformation_Centrals(double M_infall, double[:] t, double[:] delta_t, double[:] z, double[:] M_acc, double MaxGas, double T_quench, double Tau_f, str SFR_Model = "CE", int Scatter_On = 1):
    ...
    for i in range(N):
        ...
        if T_quench < t[i] or i == 0:
            ...
            #G19_DPL
            if SFR_Model_int == 6:
                M_n = 10.65+ 0.33*z[i] - 0.08*(z[i]**2) #logMsun
                Norm = c_pow(10, 0.69+ 0.71*z[i] - 0.088*(z[i]**2)) #SFR peak
                Alpha = 1.0 - 0.022*z[i] + 0.009*(z[i]**2) #low mass slope 
                Beta = 1.8 - 1.0*z[i] + 0.1*(z[i]**2)#high mass slope
                MperY = 2*Norm*c_pow( c_pow(10, -Alpha*(M_out[i]-M_n)) + c_pow(10, Beta*(M_out[i]-M_n)),-1) #SFR
                log10MperY = c_log10(MperY) #logSFR
            #Test
            if SFR_Model_int == 7:
                M_n = 10.7+ 0.4*z[i] - 0.075*(z[i]**2) #logMsun
                Norm = c_pow(10, 0.7 + 0.74*z[i] - 0.087*(z[i]**2)) #SFR peak
