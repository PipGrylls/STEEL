...
##DarkMatterToStellarMassStart #moster 2013
#No `@jit` here. It was a bare `@jit`, and this function takes
#`Paramaters`, a Python dict, which numba cannot type -- so it always
#fell back to object mode and never accelerated anything, it only added
#compilation overhead and a deprecation warning. numba 0.59 made bare
#`@jit` mean `nopython=True`, at which point the same call raises
#
#    TypingError: non-precise type pyobject
#    ... Cannot determine Numba type of <class 'dict'>
#
#and the function stops working entirely. Removing the decorator is
#both the fix and a no-op on the numerics.
...
def PrepareToSave(RunParam_List):
    #This used to shell out to `os.system("rm -r" + path)` -- note the
    #missing space, which makes it `rm -r/path/...`, an unknown option.
    #It therefore always failed, so stale output directories were never
    #cleared and a re-run silently mixed new output with old. The
    #following `mkdir` (also without -p) then failed too. shutil/os do
    #the same job without a shell, and without the quoting hazard the
    #string concatenation carried.
