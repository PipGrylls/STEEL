...
#        'veldisperr',
#        'raSDSS7',
#df = pd.read_csv("/home/ssp1e17/Documents/STEEL/Data/Observational/Lorenzo_SDSS/new_catalog_cModel.dat", delim_whitespace=True)
...
HM_Len = LenticularGalaxy.MhaloL
HM_Ell = EllipticalGalaxy.MhaloL
HM_Late = LateGalaxy.MhaloL

SM_Len = LenticularGalaxy.MsMendSerExp
SM_Ell = EllipticalGalaxy.MsMendSerExp
SM_Late = LateGalaxy.MsMendSerExp


hm_binwidth = 0.2
hm_bins = np.arange(10,15,hm_binwidth)
...
for i in range(len(hm_bins)):
    ...
    if numtempEll.empty:
        ...
        Num_Ell = weighted.median(numtempEll, wtmpEll)
    if numtempLen.empty:
        Num_Len = float('NaN')
    else:
        Num_Len = weighted.median(numtempLen, wtmpLen)
    if numtempLate.empty:
        Num_Late = float('NaN')
    else:
        Num_Late = weighted.median(numtempLate, wtmpLate)
...
x1 = hm_bins[8:-1]
y1 = SM_Ell[8:-1]
fractionEll = Frac_Ell[8:-1] # using these indices to avoid NaN values
...
#plt.plot(hm_bins, SM_Ell, label = 'Elliptical')
#plt.plot(hm_bins, SM_Len, label = 'Lenticular')
#plt.plot(hm_bins, SM_Late, label = 'Spiral')
#plt.legend()
#plt.ylabel("$log_{10}$ $M_*$ [$M_\odot$]")
#plt.xlabel("$log_{10}$ $M_h$ [$M_\odot$]")
#plt.show()
