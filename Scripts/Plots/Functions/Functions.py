...
def StarFormationRate(SM, z, Parameters, ScatterOn =True, Quenching = False, P_ellip = None):
    ...
    z: Redshift
    Parameters: a dict containing the key 'SFR_Model'
    ScatterOn: Bool for scatter models
    Returns:
    SFR: Star formation rate [log10 Msun yr-1]
    """
    #Schreiber 2015
    if Parameters['SFR_Model'] == 'S15':
    ...
    elif Parameters['SFR_Model'] == "CE":
        #Grylls19 continuity fit
...
def GetGasMass(SM, z, HM, Parameters):
    """
...
def StarFormation(SM_Sat, TTZ0, Tdyf, z_infall, z_return, z_all, HM_infall, AvaHaloMass, Paramaters, StripFactor = None, Stripping = False):
    """
    ...
    Args:
        ...
        StripFactor: log10 stripping fraction from infall
        Stripping: Bool to switch stripping on or off
    Returns:
    ...
    Tau_f = Tau_f*np.power(1+z_infall, -3/2)
    
    
    
    T_quench = t[0] - Tau_d #Quenchtime
    if Paramaters['PreProcessing']:
        Mean_SM_Sat = np.log10(np.mean(np.power(10, SM_Sat)))
        if   Mean_SM_Sat<6: PP_Frac = 0.6
        elif Mean_SM_Sat>8: PP_Frac = 0.3
        else: PP_Frac = 0.6 - 0.3*((Mean_SM_Sat - 6)/2)
        T_quench[:int(PP_Frac*len(T_quench))] = t[0]
...
def StellarMassLoss(HM_c, HM_s, SM, TTZ0, Tdyf, factor_only = False):
    """
    Args:
    ...
    """
    #Assuming the stripping is smooth ove rthe accretion time create a factor from 0 (infall) to 1 (merge) unless it never mergers in which case factor will end before 1
    Factor = (TTZ0[0] - TTZ0)/Tdyf
    ...
    if len(np.shape(SM)) == 1:
        return np.add.outer(Strip_f, SM) #Return stripped SM
    else:
        Strip_f = np.multiply.outer(Strip_f, np.ones_like(SM[0]))
        return np.add(Strip_f, SM)
...
def DynamicalFriction(HostHaloMass, SatiliteHaloMass, Redshift, Paramaters):
    ...
    if Paramaters['AltDynamicalTimeB']:
        Tdyn = Tdyn * (1/(1+Redshift))
    ...
    #A_e=(0.94*(NormalRnd**0.6) + 0.6)
    #tdyf = 1.17*((VR**2)*np.sqrt(np.power(10, -14.34693206746732+HostHaloMass)/VR)/(np.power(10, -14.34693206746732+SatiliteHaloMass)*np.log(1+MassRatio)))
    #return tdyf
...
def SaveData_3(AvaHaloMass, AnalyticalModel_SMF, Surviving_Sat_SMF_MassRange, RunParam):
    ...
    np.save(OutputFolder +"RunParam_{}/Figure3_AnalyticalModel_SMF.npy".format("".join(("{}_".format(i) for i in RunParam))), AnalyticalModel_SMF)
    np.save(OutputFolder +"RunParam_{}/Figure3_Surviving_Sat_SMF_MassRange.npy".format("".join(("{}_".format(i) for i in RunParam))), Surviving_Sat_SMF_MassRange)
def SaveData_4_6(AvaHaloMass, AnalyticalModelFrac_, AnalyticalModelNoFrac_, SM_Cuts, RunParam):
    """Figure 4 + 6"""
    np.save(OutputFolder +"RunParam_{}/Figure4_6_AvaHaloMass.npy".format("".join(("{}_".format(i) for i in RunParam))), AvaHaloMass)
    np.save(OutputFolder +"RunParam_{}/Figure4_6_AnalyticalModelFrac_.npy".format("".join(("{}_".format(i) for i in RunParam))), AnalyticalModelFrac_)
    np.save(OutputFolder +"RunParam_{}/Figure4_6_AnalyticalModelNoFrac_.npy".format("".join(("{}_".format(i) for i in RunParam))), AnalyticalModelNoFrac_)
    np.save(OutputFolder +"RunParam_{}/Figure4_6_SM_Cuts.npy".format("".join(("{}_".format(i) for i in RunParam))), SM_Cuts)
def SaveData_5(Sat_SMHM, Sat_Parent_SMHM, RunParam):
    """Figure 5"""
    np.save(OutputFolder +"RunParam_{}/Figure5_Sat.npy".format("".join(("{}_".format(i) for i in RunParam))), Sat_SMHM)
    np.save(OutputFolder +"RunParam_{}/Figure5_Sat_Parent_SMHM.npy".format("".join(("{}_".format(i) for i in RunParam))), Sat_Parent_SMHM)
def SaveData_7(Mergers, Minor_Mergers, z, RunParam):
    """Figure 7"""
    np.save(OutputFolder +"RunParam_{}/Figure7_Mergers.npy".format("".join(("{}_".format(i) for i in RunParam))), Mergers)
    np.save(OutputFolder +"RunParam_{}/Figure7_Minor_Mergers.npy".format("".join(("{}_".format(i) for i in RunParam))), Minor_Mergers)
    np.save(OutputFolder +"RunParam_{}/Figure7_z.npy".format("".join(("{}_".format(i) for i in RunParam))), Minor_Mergers)
def SaveData_8(AvaHaloMass, P_Elliptical, RunParam):
    """Figure 8"""
    np.save(OutputFolder +"RunParam_{}/Figure8_AvaHaloMass.npy".format("".join(("{}_".format(i) for i in RunParam))), AvaHaloMass)
    np.save(OutputFolder +"RunParam_{}/Figure8_P_Elliptical.npy".format("".join(("{}_".format(i) for i in RunParam))), P_Elliptical)
def SaveData_9(AvaHaloMass, z, Analyticalmodel_SI, RunParam):
    """Figure 9"""
    np.save(OutputFolder +"RunParam_{}/Figure9_AvaHaloMass.npy".format("".join(("{}_".format(i) for i in RunParam))), AvaHaloMass)
    np.save(OutputFolder +"RunParam_{}/Figure9_z.npy".format("".join(("{}_".format(i) for i in RunParam))), z)
    np.save(OutputFolder +"RunParam_{}/Figure9_Analyticalmodel_SI.npy".format("".join(("{}_".format(i) for i in RunParam))), Analyticalmodel_SI)
def SaveData_10(AvaHaloMass, AnalyticalModel_SMF, Surviving_Sat_SMF_MassRange, RunParam):
    """Figure 10"""
    np.save(OutputFolder +"RunParam_{}/Figure10_AvaHaloMass.npy".format("".join(("{}_".format(i) for i in RunParam))), AvaHaloMass)
    np.save(OutputFolder +"RunParam_{}/Figure10_AnalyticalModel_SMF.npy".format("".join(("{}_".format(i) for i in RunParam))), AnalyticalModel_SMF)
    np.save(OutputFolder +"RunParam_{}/Figure10_Surviving_Sat_SMF_MassRange.npy".format("".join(("{}_".format(i) for i in RunParam))), Surviving_Sat_SMF_MassRange)
def SaveData_SMFhz(AvaHaloMass, AnalyticalModel_SMF_Highz, Surviving_Sat_SMF_MassRange, RunParam):
    """Figure 3"""
    np.save(OutputFolder +"RunParam_{}/SMFhz_AvaHaloMass.npy".format("".join(("{}_".format(i) for i in RunParam))), AvaHaloMass)
    np.save(OutputFolder +"RunParam_{}/SMFhz_AnalyticalModel_SMF_Highz.npy".format("".join(("{}_".format(i) for i in RunParam))), AnalyticalModel_SMF_Highz)
    np.save(OutputFolder +"RunParam_{}/SMFhz_Surviving_Sat_SMF_MassRange.npy".format("".join(("{}_".format(i) for i in RunParam))), Surviving_Sat_SMF_MassRange)
def SaveData_z_infall(Surviving_Sat_SMF_MassRange, z, z_infall, RunParam):
    np.save(OutputFolder +"RunParam_{}/z_infall_Surviving_Sat_SMF_MassRange.npy".format("".join(("{}_".format(i) for i in RunParam))), Surviving_Sat_SMF_MassRange)
    np.save(OutputFolder +"RunParam_{}/z_infall_z.npy".format("".join(("{}_".format(i) for i in RunParam))), z)
    np.save(OutputFolder +"RunParam_{}/z_infall.npy".format("".join(("{}_".format(i) for i in RunParam))), z_infall)
def SaveData_sSFR(Surviving_Sat_SMF_MassRange, sSFR_Range, Satilite_sSFR, RunParam):
    np.save(OutputFolder +"RunParam_{}/sSFR_Surviving_Sat_SMF_MassRange.npy".format("".join(("{}_".format(i) for i in RunParam))), Surviving_Sat_SMF_MassRange)
    np.save(OutputFolder +"RunParam_{}/sSFR_Range.npy".format("".join(("{}_".format(i) for i in RunParam))), sSFR_Range)
    np.save(OutputFolder +"RunParam_{}/Satellite_sSFR.npy".format("".join(("{}_".format(i) for i in RunParam))), Satilite_sSFR)
def SaveData_Sat_SMHM(z, SatHaloMass, AvaHaloMass, Surviving_Sat_SMF_MassRange, Sat_SMHM, Sat_SMHM_Host, RunParam):
    np.save(OutputFolder +"RunParam_{}/Sat_SMHM_z.npy".format("".join(("{}_".format(i) for i in RunParam))), z)
    np.save(OutputFolder +"RunParam_{}/Sat_SMHM_SatHaloMass.npy".format("".join(("{}_".format(i) for i in RunParam))), SatHaloMass)
    np.save(OutputFolder +"RunParam_{}/Sat_SMHM_AvaHaloMass.npy".format("".join(("{}_".format(i) for i in RunParam))), AvaHaloMass)
    np.save(OutputFolder +"RunParam_{}/Sat_SMHM_Surviving_Sat_SMF_MassRange.npy".format("".join(("{}_".format(i) for i in RunParam))), Surviving_Sat_SMF_MassRange)
    np.save(OutputFolder +"RunParam_{}/Sat_SMHM_Sat_SMHM.npy".format("".join(("{}_".format(i) for i in RunParam))), Sat_SMHM)
    np.save(OutputFolder +"RunParam_{}/Sat_SMHM_Sat_SMHM_Host.npy".format("".join(("{}_".format(i) for i in RunParam))), Sat_SMHM_Host)
def SaveData_Mergers(Accretion_History, z, AvaHaloMass, Surviving_Sat_SMF_MassRange, RunParam):
    np.save(OutputFolder +"RunParam_{}/Mergers_Accretion_History".format("".join(("{}_".format(i) for i in RunParam))), Accretion_History)
    np.save(OutputFolder +"RunParam_{}/Mergers_z".format("".join(("{}_".format(i) for i in RunParam))), z)
    np.save(OutputFolder +"RunParam_{}/Mergers_AvaHaloMass".format("".join(("{}_".format(i) for i in RunParam))), AvaHaloMass)
    np.save(OutputFolder +"RunParam_{}/Mergers_Surviving_Sat_SMF_MassRange.npy".format("".join(("{}_".format(i) for i in RunParam))), Surviving_Sat_SMF_MassRange)
def SaveData_Pair_Frac(Pair_Frac, z, AvaHaloMass, Surviving_Sat_SMF_MassRange, RunParam):
    np.save(OutputFolder +"RunParam_{}/Pair_Frac_Pair_Frac".format("".join(("{}_".format(i) for i in RunParam))), Pair_Frac)
    np.save(OutputFolder +"RunParam_{}/Pair_Frac_z".format("".join(("{}_".format(i) for i in RunParam))), z)
    np.save(OutputFolder +"RunParam_{}/Pair_Frac_AvaHaloMass".format("".join(("{}_".format(i) for i in RunParam))), AvaHaloMass)
    np.save(OutputFolder +"RunParam_{}/Pair_Frac_Surviving_Sat_SMF_MassRange.npy".format("".join(("{}_".format(i) for i in RunParam))), Surviving_Sat_SMF_MassRange)
def SaveData_Sat_Env_Highz(AvaHaloMass, z, AnalyticalModel_Cuts_Frac_highz, AnalyticalModel_Cuts_NoFrac_highz, SM_Cuts, RunParam):
    np.save(OutputFolder +"RunParam_{}/Sat_Env_Highz_AvaHaloMass.npy".format("".join(("{}_".format(i) for i in RunParam))), AvaHaloMass)
    np.save(OutputFolder +"RunParam_{}/Sat_Env_Highz_z".format("".join(("{}_".format(i) for i in RunParam))), z)
    np.save(OutputFolder +"RunParam_{}/Sat_Env_Highz_AnalyticalModelFracHighz.npy".format("".join(("{}_".format(i) for i in RunParam))), AnalyticalModel_Cuts_Frac_highz)
    np.save(OutputFolder +"RunParam_{}/Sat_Env_Highz_AnalyticalModelNoFracHighz.npy".format("".join(("{}_".format(i) for i in RunParam))), AnalyticalModel_Cuts_NoFrac_highz)
    np.save(OutputFolder +"RunParam_{}/Sat_Env_Highz_SM_Cuts.npy".format("".join(("{}_".format(i) for i in RunParam))), SM_Cuts)
def SaveData_Raw_Richness(AvaHaloMass, z, Surviving_Sat_SMF_MassRange, Surviving_Sat_SMF_Weighting_highz, RunParam):
    np.save(OutputFolder +"RunParam_{}/Raw_Richness_AvaHaloMass.npy".format("".join(("{}_".format(i) for i in RunParam))), AvaHaloMass)
    np.save(OutputFolder +"RunParam_{}/Raw_Richness_Highz_z".format("".join(("{}_".format(i) for i in RunParam))), z)
    np.save(OutputFolder +"RunParam_{}/Raw_Richness_Surviving_Sat_SMF_MassRange.npy".format("".join(("{}_".format(i) for i in RunParam))), Surviving_Sat_SMF_MassRange)
    np.save(OutputFolder +"RunParam_{}/Raw_Richness_Surviving_Sat_SMF_Weighting_highz.npy".format("".join(("{}_".format(i) for i in RunParam))), Surviving_Sat_SMF_Weighting_highz)
def SaveData_MultiEpoch_SubHalos(z, SatHaloMass, SurvivingSubhalos_z_z, RunParam):
    np.save(OutputFolder +"RunParam_{}/MultiEpoch_SubHalos_z.npy".format("".join(("{}_".format(i) for i in RunParam))), z)
    np.save(OutputFolder +"RunParam_{}/MultiEpoch_SatHaloMass.npy".format("".join(("{}_".format(i) for i in RunParam))), SatHaloMass)
    np.save(OutputFolder +"RunParam_{}/MultiEpoch_SurvivingSubhalos_z_z.npy".format("".join(("{}_".format(i) for i in RunParam))), SurvivingSubhalos_z_z)
def SaveData_Pair_Frac_Halo(Pair_Frac_Halo, Accretion_History_Halo, z, AvaHaloMass, SatHaloMass, RunParam):
    np.save(OutputFolder +"RunParam_{}/Pair_Frac_Halo_z.npy".format("".join(("{}_".format(i) for i in RunParam))), z)
    np.save(OutputFolder +"RunParam_{}/Pair_Frac_Halo_Pair_Frac_Halo.npy".format("".join(("{}_".format(i) for i in RunParam))), Pair_Frac_Halo)
    np.save(OutputFolder +"RunParam_{}/Pair_Frac_Halo_Accretion_History_Halo.npy".format("".join(("{}_".format(i) for i in RunParam))), Accretion_History_Halo)
    np.save(OutputFolder +"RunParam_{}/Pair_Frac_Halo_AvaHaloMass.npy".format("".join(("{}_".format(i) for i in RunParam))), AvaHaloMass)
    np.save(OutputFolder +"RunParam_{}/Pair_Frac_Halo_SatHaloMass.npy".format("".join(("{}_".format(i) for i in RunParam))), SatHaloMass)
def SaveData_Total_Starformation(AvaHaloMass, z, Surviving_Sat_SMF_MassRange, Total_StarFormation_Means, Total_StarFormation_Std, RunParam):
    np.save(OutputFolder +"RunParam_{}/Total_Starformation_AvaHaloMass.npy".format("".join(("{}_".format(i) for i in RunParam))), AvaHaloMass)
    np.save(OutputFolder +"RunParam_{}/Total_Starformation_Surviving_Sat_SMF_MassRange.npy".format("".join(("{}_".format(i) for i in RunParam))), Surviving_Sat_SMF_MassRange)
    np.save(OutputFolder +"RunParam_{}/Total_Starformation_Total_StarFormation_Means.npy".format("".join(("{}_".format(i) for i in RunParam))), Total_StarFormation_Means)
    np.save(OutputFolder +"RunParam_{}/Total_Starformation_Total_StarFormation_Std.npy".format("".join(("{}_".format(i) for i in RunParam))), Total_StarFormation_Std)
    np.save(OutputFolder +"RunParam_{}/Total_Starformation_z.npy".format("".join(("{}_".format(i) for i in RunParam))), z)
#==========================Loading Output=======================================
def LoadData_3(RunParam_List):
    """Figure 3"""
    #print('\nOutputFolder 3 = '+OutputFolder +"RunParam_{}/Figure3_AvaHaloMass.npy".format("".join(("{}_".format(i) for i in RunParam_List[0])))+'\n')
...
def LoadData_sSFR(RunParam):
    Surviving_Sat_SMF_MassRange = np.load(OutputFolder +"RunParam_{}/sSFR_Surviving_Sat_SMF_MassRange.npy".format("".join(("{}_".format(i) for i in RunParam[0]))))
    sSFR_Range = np.load(OutputFolder +"RunParam_{}/sSFR_Range.npy".format("".join(("{}_".format(i) for i in RunParam[0]))))
    Satellite_sSFR = np.load(OutputFolder +"RunParam_{}/Satellite_sSFR.npy".format("".join(("{}_".format(i) for i in RunParam[0]))))
    return Surviving_Sat_SMF_MassRange, sSFR_Range, Satellite_sSFR
def LoadData_Sat_SMHM(RunParam):
    #print('\nOutputFolder = '+OutputFolder +"RunParam_{}/Sat_SMHM_z.npy".format("".join(("{}_".format(i) for i in RunParam[0])))+'\n')
    z = np.load(OutputFolder +"RunParam_{}/Sat_SMHM_z.npy".format("".join(("{}_".format(i) for i in RunParam[0]))))
    SatHaloMass = np.load(OutputFolder +"RunParam_{}/Sat_SMHM_SatHaloMass.npy".format("".join(("{}_".format(i) for i in RunParam[0]))))
    AvaHaloMass = np.load(OutputFolder +"RunParam_{}/Sat_SMHM_AvaHaloMass.npy".format("".join(("{}_".format(i) for i in RunParam[0]))))
    Surviving_Sat_SMF_MassRange = np.load(OutputFolder +"RunParam_{}/Sat_SMHM_Surviving_Sat_SMF_MassRange.npy".format("".join(("{}_".format(i) for i in RunParam[0]))))
    Sat_SMHM = np.load(OutputFolder +"RunParam_{}/Sat_SMHM_Sat_SMHM.npy".format("".join(("{}_".format(i) for i in RunParam[0]))))
    Sat_SMHM_Host = np.load(OutputFolder +"RunParam_{}/Sat_SMHM_Sat_SMHM_Host.npy".format("".join(("{}_".format(i) for i in RunParam[0]))))
...
def LoadData_Total_Starformation(RunParam):
    AvaHaloMass = np.load(OutputFolder +"RunParam_{}/Total_Starformation_AvaHaloMass.npy".format("".join(("{}_".format(i) for i in RunParam[0]))))
    Surviving_Sat_SMF_MassRange = np.load(OutputFolder +"RunParam_{}/Total_Starformation_Surviving_Sat_SMF_MassRange.npy".format("".join(("{}_".format(i) for i in RunParam[0]))))
    Total_StarFormation_Means = np.load(OutputFolder +"RunParam_{}/Total_Starformation_Total_StarFormation_Means.npy".format("".join(("{}_".format(i) for i in RunParam[0]))))
    Total_StarFormation_Std = np.load(OutputFolder +"RunParam_{}/Total_Starformation_Total_StarFormation_Std.npy".format("".join(("{}_".format(i) for i in RunParam[0]))))
    z = np.load(OutputFolder +"RunParam_{}/Total_Starformation_z.npy".format("".join(("{}_".format(i) for i in RunParam[0]))))
